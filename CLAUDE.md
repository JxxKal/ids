...
- **Burst-Vorsorge (seit 2026-09, OT-Vorfall)**: Eine Suricata-Stream-Engine-SID feuerte mit 10.000 Alerts/s, `alerts-raw` hielt zeitbasiert 88 GB, Platte voll, Kafka tot, Container weg, Pipeline drei Tage blind — im Frontend nur „Threat Level 0", weil alert-manager die Flut per Dedup schluckte. Vier Sicherungen, die zusammen greifen: (1) `snort-bridge` drosselt VOR Kafka (`snort-bridge/src/throttle.py`: Engine-Events SID 2200000–2299999 verwerfen, Cooldown pro SID+Verbindung, Token-Bucket 50/s; `SNORT_*`-Vars in `.env`). (2) Kafka hat Byte-Caps pro Partition — Broker-Default 2 GB in `docker-compose.yml`, Topic-Caps in `infra/kafka/init-topics.sh` (Summe ≈ 17 GB worst case), Segmente 128 MB, Cleanup alle 30 s. (3) `DISK_SPACE_001`/`DISK_EMERGENCY_001` nennen die größten Docker-Volumes (`docker system df -v` aus dem api-Container), damit die Meldung dorthin zeigt, wo der Platz liegt. (4) `cyjan-stack-watch.timer` (5 min) bringt fehlende/gestoppte Services per `docker compose up -d` zurück; `touch /run/cyjan-stack-watch.pause` für Wartung. Bei einer Flut in der Diagnose immer zuerst `sudo docker system df -v` — DB-Retention hilft nicht, wenn der Platz in Kafka oder MinIO liegt.
...
- **Reverse-Tap-Update** (seit v2.3.4): Master-Update-ZIP enthält jetzt unter `tap-update/` zusätzlich `images-tap.tar.zst` + Manifest + aktuelles `docker-compose.tap.yml`. master-uplink serviert die Files mTLS-protected unter `/tap-update/<file>`. Auf einem gepairten Tap reicht `sudo cyjan-tap update --from-master` — pull-basiert, sha256-verifiziert, `docker load` + `compose up -d --force-recreate`, Outage-Buffer fängt die ~30 s Stack-Restart-Lücke ab. Voraussetzung am Master: System-Update wurde eingespielt (sonst leeres `/opt/ids/tap-update/` → 404).
- **Console-Updater** (seit v2.5.10): `cyjan-update` am Master-Host für SSH-Workflow. `cyjan-update` zeigt Ist/Latest-Diff, `cyjan-update apply [tag]` lädt das Update-ZIP aus GitHub-Releases (System-Proxy), `zstd | docker load`, `git pull` (oder rsync aus ZIP falls Repo dirty/missing), `post-update.sh` und `docker compose up -d --force-recreate`. Air-Gap-Variante via `--from-zip /pfad.zip`. Symmetrisch zu `cyjan-tap update --from-master` am Tap.
- **Settings-Migration** (seit v2.5.9): Settings → Host-Migration in der GUI. Export liefert ein `.cysb`-Bundle (tar.gz) mit DB-Settings-Tabellen (users, system_config, known_networks, host_info, taps, …), Heuristik-Volume, Master-CA + Privatekey, ML-Config. Import-Preview matched Quell-NICs gegen Ziel-NICs (Listen-File `/etc/cyjan/host-interfaces.json` aus `cyjan-host-interfaces.timer`). Strategie: TRUNCATE + INSERT, Quelle gewinnt. Re-Auth via Admin-Passwort. Quell-Host muss nach erfolgreichem Import abgeschaltet werden, sonst sehen die Taps zwei Master. Operative Daten (Alerts/Flows/ML-Modell) sind NICHT im Bundle — dafür ist `/api/maintenance/backup` zuständig.
