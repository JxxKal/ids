...
Die Kommunikation in Gegenrichtung (Master → Tap) für Rule-/Suppression-
Sync läuft NICHT über diesen Socket sondern über den separaten REST-
Pull-Endpoint im api-Container. Hält den Uplink simpel und bidirektional-
free.
"""
...
class TapAuth:
    ...
    async def lookup(self, fingerprint: str) -> dict | None:
        ...
        return {"id": str(row["id"]), "name": row["name"], "status": "active"}
